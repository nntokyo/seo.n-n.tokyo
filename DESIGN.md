...
5. **完全インタラクティブ保証 (Zero Dead Buttons)**:
   - すべてのタブ・トグル・アクションボタン（`総合診断`, `AEO / LLMO`, `メタ不具合` 等）は、クリック時に対応するプレビューカードやタブパネルを即座に動的切り替えすること。
   - すべてのフィーチャーカード（矢印アイコン付き等）は、クリック可能なリンク（`<Link href="...">`）またはアンカーリンクとして機能し、対応ツールや詳細セクションへスムーズ遷移すること。
   - すべてのコードブロックのコピーボタンは、個別スニペット単位で独立したフィードバック（`copiedId` による Copied 表示）を提供すること。
