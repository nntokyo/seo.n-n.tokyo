...
- 正常に取得した診断結果はURL単位で24時間、サーバー上の非公開データ領域へ保存し、プロセス再起動後も再利用します。
- 同じURLへの同時リクエストは1本のGoogle APIリクエストへ集約します。
- HTTP 4xx（特に日次上限を示す429）は再試行しても成功しないため、その場では再試行しません。HTTP 5xxや一時的な通信障害だけを1回再試行します。
- 429発生時は取得済みの期限切れキャッシュがあれば、その取得日時を保持したまま暫定表示します。キャッシュがなければ、Google Cloud側のクォータ回復または上限変更が必要であることを画面へ表示します。
- 公開ハブの既定対象であるシステム所有サイトは、未ログイン表示でもサーバー側のPageSpeed専用キーを使用します。対象ホストは `PUBLIC_SITE_URL`（未設定時は本番FQDN）と完全一致するものに限定します。
- システム所有サイト以外で、プロジェクト固有キーも管理者権限もない場合はGoogleへキーレスリクエストを送信せず、個別キーの設定を案内します。
...
Google Gemini 3.8 Flashを活用し、検出された課題を解決するための**「自然で検索意図に沿った日本語修正テキスト」**および**「Next.js等の実装コード」**をオンデマンドで生成します。

- **既定モデル**: `gemini-3.8-flash`（Stable / GA）
- **環境変数**: `GEMINI_MODEL` でモデルIDを上書き可能
- **移行上の注意**: Gemini 3系では `temperature` / `top_p` / `top_k` など旧サンプリングパラメータを指定せず、モデル既定値を利用します。
- **Thinking**: Gemini 3.8 Flashの既定thinking levelは `medium`。本機能では現時点で明示指定せず、Googleの既定値を使用します。
- **出力**: 現行実装はJSON形式をプロンプトで要求し、レスポンス本文をJSONとして検証・パースします。Structured Outputsの導入は必要に応じて別途検討します。
...
2. **システム環境変数（`.env`）のスーパーアドミン限定フォールバック**:
   ...
   - 一般ユーザーが登録した外部サイトでは、システム環境変数のトークンを勝手に消費・流用できないよう遮断し、自身のGoogle API設定を促すUIを表示。公開ハブのシステム所有サイトだけは、ホスト完全一致と24時間キャッシュを条件にPageSpeed専用キーの利用を許可する。
