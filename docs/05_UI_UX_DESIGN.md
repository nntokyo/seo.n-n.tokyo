...
### 2.8 マイページ・アカウント管理 (`/account`) [SCR-29]
ログインユーザー自身のアカウント情報、権限、メールアドレス変更、認証ステータス、パスワード変更、管理プロジェクトの統計を統合管理する専用画面です。
...
|  +-- [ 基本プロフィール情報 ] ------------------------------------------------------------------+ |
|  | 名前: 山田 太郎                [ ✏️ 表示名を変更 ]                                             | |
|  | メールアドレス: yamada@example.com (認証済み ✅)   [ ✉️ メールアドレス変更 ]                  | |
|  | ※ 未認証の場合: ⚠️ 未認証 [ 6桁の確認コードを入力: [ 123456 ] (認証完了) (コード再送) ]         | |
|  | ログイン方式: Google OAuth / メール・パスワード                                               | |
|  | システム権限: スーパー管理者 (ADMIN) / 一般メンバー (MEMBER)                                    | |
|  | アカウント作成日: 2026/09/10                                                                  | |
|  +----------------------------------------------------------------------------------------------+ |
|                                                                                                   |
|  +-- [ セキュリティ & パスワード変更 ] ---------------------------------------------------------+ |
...
|                                                                                                   |
|  +-- [ 所有プロジェクトサマリー ] --------------------------------------------------------------+ |
|  | 監視中ドメイン数: 3 件      最新平均スコア: 86 / 100                                            | |
|  | ※ メールアドレスが未認証の場合、プロジェクト機能は自動的にロックされます                      | |
|  | • example.com (最終スコア: 92点)   [ ダッシュボードを開く → ]                                  | |
|  | [ プロジェクト一覧を管理する (SCR-15) → ]                                                      | |
|  +----------------------------------------------------------------------------------------------+ |
+---------------------------------------------------------------------------------------------------+
```
...
### 2.9 プラットフォーム管理画面 (`/admin`) [SCR-30]
システム管理者（`role === 'ADMIN'`）専用の全体監視コンソールです。全テナントのユーザー情報、メール認証率、全プロジェクトの診断推移、システム環境変数の状態を一元把握・制御します。

```
+---------------------------------------------------------------------------------------------------+
| 🛡️ プラットフォーム管理コンソール (SCR-30)                    [ ⚙️ システム設定 ]  [ 👤 マイページ ]  |
+---------------------------------------------------------------------------------------------------+
|  [ 4連 KPI メトリクスカード ]                                                                     |
|  +---------------------+---------------------+---------------------+---------------------------+  |
|  | 総登録ユーザー数    | メール認証完了率    | 総管理プロジェクト  | 累計診断実行回数          |  |
|  |     128 名          |      94.5 %         |     312 件          |     1,842 回              |  |
|  |  🟢 +12名 (今週)    |  🟢 健全            |  🟢 ドメイン稼働中  |  ⚡ 平均応答: 1.4秒       |  |
|  +---------------------+---------------------+---------------------+---------------------------+  |
|                                                                                                   |
|  --- 👥 プラットフォーム全ユーザー一覧 ----------------------------------------------------------  |
|  +---------------------------------------------------------------------------------------------+  |
|  | 氏名         | メールアドレス      | 認証状況 | ログイン方式 | 権限ロール   | 登録日時      |  |
|  |-------------|---------------------|----------|--------------|--------------|---------------|  |
|  | 管理者      | admin@n-n.tokyo     | 認証済み | Google OAuth | ADMIN (管理) | 2026/09/10    |  |
|  | 田中 健一   | tanaka@example.com  | 認証済み | LOCAL        | MEMBER       | 2026/09/12    |  |
|  | 鈴木 一郎   | suzuki@test.com     | ⚠️ 未認証| LOCAL        | MEMBER       | 2026/09/13    |  |
|  +---------------------------------------------------------------------------------------------+  |
|                                                                                                   |
|  --- 🌐 全登録プロジェクト・ドメイン稼働一覧 ----------------------------------------------------  |
|  +---------------------------------------------------------------------------------------------+  |
|  | プロジェクト名 | 監視URL                | 所有者ID   | 最新スコア | 診断回数 | アクション     |  |
|  |----------------|------------------------|------------|------------|----------|----------------|  |
|  | 公式ポータル   | https://seo.n-n.tokyo  | usr_admin  | 94点       | 28回     | [ 詳細確認 → ] |  |
|  | ECストア       | https://shop.demo.com  | usr_tanaka | 82点       | 12回     | [ 詳細確認 → ] |  |
|  +---------------------------------------------------------------------------------------------+  |
+---------------------------------------------------------------------------------------------------+
```
